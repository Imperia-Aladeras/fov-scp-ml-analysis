# Informe individual SCP vs ML — 10461_Garcia_Millan

**Fecha del analisis:** 14/07/2026
**Cliente:** ID_CLIENT=10461 | Fichero: `TA_FOV_SCP_ML_10461_Garcia_Millan.csv`
**Batch/Run:** ID_BATCH=[63] | ID_RUN_STAGING=[61] | SOURCE_RUN_ID=[1]
...
Este cliente tiene **1.486 series candidatas**, pero **ninguna es comparable** en ningun periodo analizado. No es un error del pipeline: es un caso valido de cobertura que requiere diagnostico (ver seccion 2). No se reportan metricas de performance porque no existen series validas sobre las que calcularlas.

---

## 2. Cobertura

Series candidatas (universo de cobertura, `HAS_BASE_CANDIDATE=1`): **1.486**.

Distribucion original de `COMPARISON_STATUS` (categorias del CSV, sin modificar):

| COMPARISON_STATUS | N | % sobre candidatas |
|---|---|---|
| NOT_COMPARABLE_MISSING_VALIDATION | 1.486 | 100.0% |

Exclusiones ML reales (`HAS_ML_EXCLUDED=1`, no varia por periodo): **0** (0.0% sobre candidatas).
...
| M5 | 1.486 | 0 | 0.0% |
| M6 | 1.486 | 0 | 0.0% |
| RECENT_3M | 1.486 | 0 | 0.0% |
| OLDER_3M | 1.486 | 0 | 0.0% |
| 6M | 1.486 | 0 | 0.0% |
...
`COMPARISON_STATUS='NOT_COMPARABLE_ML_EXCLUDED'`: 0 filas. `HAS_ML_EXCLUDED=1` (recuento real): 0 filas. La diferencia (0) corresponde a exclusiones ML "tapadas" por otro `COMPARISON_STATUS` de mayor precedencia (p.ej. falta tambien SCP).
...
No es posible concluir sobre la mejora de ML frente a SCP para este cliente: ninguna de sus 1.486 series candidatas es comparable (ver seccion 2 para el motivo). Esto es un resultado de cobertura, no de performance.
