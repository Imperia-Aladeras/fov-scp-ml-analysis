...
def readme_blocks(result: ClientAnalysisResult) -> list[tuple[str, pd.DataFrame]]:
    ...
    lines = [
        ...
        "Universo de cobertura: todas las filas candidatas (HAS_BASE_CANDIDATE=1).",
        "Universo de performance: mascara de comparabilidad ESPECIFICA de cada periodo",
        "(no se reutiliza la misma mascara ni unicamente COMPARISON_STATUS='COMPARABLE' para todos",
        "los periodos). Ver pestana 02_coverage_status para el detalle por periodo.",
        "",
        ...
        *[f"  {p} -> {visible_label(p)}" for p in ALL_PERIODS],
        ...
        "",
        "Regla de comparabilidad por periodo: pertenece al universo candidato, historico > 0, y",
        "forecast/error/WAPE validos para SCP y ML en ese periodo. Los trimestres y el semestre usan",
        "directamente las columnas agregadas TOTAL_* ya materializadas en el CSV: no se exige que cada",
        "mes individual sea comparable de forma aislada.",
        "",
